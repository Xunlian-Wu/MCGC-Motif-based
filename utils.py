...
def NC_loss(clusters, h1, centers, label, h2, tao):
    loss = 0
    ...
    sim_positive = torch.exp(torch.mm(h1, h2.T) / tao)
    sim_negative = torch.mul(
        torch.exp(torch.mm(h1, centers.T) / tao), indicator)
...
def CC_loss(u1, u2, h1, tao):
    ...
    sim_cross1 = torch.exp(torch.mm(z1, z2.T) / tao)
    sim_cross2 = torch.exp(torch.mm(z2, z1.T) / tao)

    sim_same1 = torch.exp(torch.mm(z1, z1.T) / tao)
    sim_same2 = torch.exp(torch.mm(z2, z2.T) / tao)
